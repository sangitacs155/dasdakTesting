...
<form name="registration" method="post" class="clearfix" action="<?php echo site_url('User/saveFriend'); ?>"novalidate="novalidate">
<?php 
foreach ($guestData AS  $value) { ?>
<div class="hide">
<div class="form-group"><label for="registration_primaryGuest_firstName" class="required">
First name&nbsp;<span class="asterisk">*</span></label><input type="text" id="registration_primaryGuest_firstName" name="registration[primaryGuest][firstName]" required="required" placeholder="First Name" class="form-control" value="<?php  echo $value->usr_firstName ;?>"></div><div class="form-group"><label for="registration_primaryGuest_lastName" class="required">
Last name&nbsp;<span class="asterisk">*</span></label><input type="text" id="registration_primaryGuest_lastName" name="registration[primaryGuest][lastName]" required="required" placeholder="Last Name" class="form-control" value="<?php  echo $value->usr_lastName ;?>"></div><div class="form-group"><label for="registration_primaryGuest_email" class="required">
Email&nbsp;<span class="asterisk">*</span></label><input type="email" id="registration_primaryGuest_email" name="registration[primaryGuest][email]" required="required" placeholder="Email" class="form-control" value="<?php  echo $value->usr_email ;?>"></div><div class="form-group"><label for="registration_primaryGuest_zipCode" class="optional">
Zip code
</label><input type="text" id="registration_primaryGuest_zipCode" name="registration[primaryGuest][zipCode]" class="zipcode-input form-control" placeholder="Zip Code" size="5" maxlength="5" value="<?php  echo $value->usr_zipCode ;?>"></div><div class="form-group"><label class="required">
...
<?php 
foreach ($guestData AS  $value) { ?>
<span class="frname"><?php echo ($value->usr_firstName) . " ".($value->usr_lastName)  ?></span>
<span class="fremail"><?php echo  $value->usr_email ?></span>
<?php } ?>
<input type="hidden" name="guestId" value = "<?php  echo $value->user_id ?>">
</div>
 <?php if(count($frndData))
   { ?>
<div class="col-md-12">
<?php
foreach ($frndData as  $val) { ?>
<span class="frname"><?php echo ($val->frnd_fname) . " ".($val->frnd_lname)  ?></span>
<span class="fremail"><?php echo  $val->frnd_email ?></span>
<?php } ?>
</div>
<?php } ?>
</div>
